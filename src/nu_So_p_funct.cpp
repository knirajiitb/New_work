...
{	
	...
	{	
		...
		k_mm= kminus_grid_pop_p[counter];  // unit is 1/nm
		k_pp= kplus_grid_pop_p[counter];   // unit is 1/nm
		...
		if((energy_p[counter] < h_bar*omega_LO)||(k_mm==k_dum))
			...
			B_minus= (((1+3*C_minus*C_minus)/2)*log(abs((1+C_minus)/(1-C_minus))) - 3*C_minus);		
		//from equation 3.22 of ramu thesis
		
		if ((k_pp == k_dum))
			B_plus = 0;
		else
			B_plus = (((1+3*C_plus*C_plus)/2)*log(abs((1+C_plus)/(1-C_plus))) - 3*C_plus);		
		//from equation 3.22 of ramu thesis
