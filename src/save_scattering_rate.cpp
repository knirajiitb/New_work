...
//------------------------------------Saving scattering rate--------------------------------------------------------
	...
	else if(geometry==2) // for 2D
	{
		...
		fprintf(fid1,"# energy           nu_rim            nu_pop         nu_npop        nu_de      nu_pz    nu_so_pop   nu_total (total relaxation time 1/second)\n");
		...
		for (int i = 0; i < points; i++)
		{
			...
			mfp[i] = mfp[i]/(1e-9);
		        ...
		        fprintf(fid1,"  %e    %e    %e    %e    %e    %e    %e    %e \n", energy_n[i], nu_ionizedimpurity[i], 
			nu_pop_total[i], nu_npop_total[i], nu_deformation[i], nu_piezoelectric[i], nu_so_pop_total[i], denom[i] );
