...
// but with calculated group velocity from band structure and DOS both calculated from DFT:
{
    ...
    if (free_e ==1)
    {
        for (int counter = 0;counter<points-1;counter++)
        {
            ...
            for (int i = 0;i<=factor-1;i++)
            {
                integral_numerator = integral_numerator + k_step * pow(((k_grid[counter]+i*k_step)/pi),2)*(v_n[counter] + 				i*dv)*h_LO[counter] / (Bfield*0.0001);
                    // Bfield is multplied with 0.0001 to convert into cgs unit
                    // =1/Bfield*int[h_LO(En)*DOS(En)*v(En)*dEn]
 
                integral_denominator = integral_denominator + k_step * pow(((k_grid[counter]+i*k_step)/pi),2)*(v_n[counter] + 				i*dv)*g_LO[counter];
    ...
    else
    {
        for (int counter = 0;counter<=points-2;counter++)
        {
            ...
            integral_numerator = integral_numerator + de * Ds_n[counter]/volume1 * v_n[counter] * h_LO[counter]/(Bfield*0.0001);
            // =1/Bfield*int[h_LO(En)*DOS(En)*v(En)*dEn]
            // Bfield is multplied with 0.0001 to convert into cgs unit

            integral_denominator = integral_denominator + de * Ds_n[counter]/volume1 * v_n[counter] * g_LO[counter];
